...
//
// scripts/verify/vendor_verify.sh holds the actual rules. This test exists so they are
...
// never reach for scripts/vendor/node_modules.
...
describe("vendored files", () => {
    test("have not drifted from their recorded hashes", () => {
        try {
            execFileSync("bash", ["scripts/verify/vendor_verify.sh"], {
                cwd: ROOT,
                encoding: "utf8",
            ...
            assert.fail(`scripts/verify/vendor_verify.sh reported drift:\n\n${err.stderr || err.message}`)
        }
    })
})
