// CM6 Terminal layer tests — run with: node --test test/js/terminal/terminal_test.mjs
//
// Three test groups:
...
//
// Run: node --test test/js/terminal/terminal_test.mjs
...
function makeMockCm6() {
    ...
    return {
        ...
        StateEffect:         { define: () => ({ of: (v) => ({ value: v }) }) },
        hoverTooltip:        (_source, _opts) => [],
        autocompletion:      (_config) => [],
        lintGutter:          () => [],
...
describe("Terminal (CM6)", () => {
    ...
    // BIRTH IS AN ACT, NOT A SIDE EFFECT. inner() once published while it
    // selected the initial buffer, so a subscriber wired before it heard a
    // half-built room and reached for organs the mount had not made yet
    // (a TDZ throw that killed the rest of mountInner). Construction is silent;
    // the mount speaks the first breath when it is whole.
    test("inner() is silent; triggerBridge speaks the first breath", () => {
        const cm6  = makeMockCm6();
        const term = new Terminal(makeEditorStub(), cm6);

        const heard = [];
        term.bridge.sub((payload) => heard.push(payload));

        term.inner();
        assert.equal(heard.length, 0, "construction published nothing");

        term.triggerBridge();
        assert.equal(heard.length, 1, "birth published once");
        assert.equal(heard[0].id, term.currentBufferId());

        // A real switch is still news — silence belongs to construction alone.
        const second = term.createBuffer("second", "rt 90");
        heard.length = 0;
        term.opBufferHandler({ op: 'select', target: second });
        assert.ok(heard.some((p) => p.id === second), "a switch still announces");
    });
