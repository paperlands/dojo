...
// — a short page that does not fill the window still reports the whole
...
// FOLLOW — arrive at the friend's line. Landing the caret is the act: the
// cursor law lights their cell; the surface hands the same line to the page
// law. Caret first, then the glide — one motion, not a jump and a slide.
// Returns whether the line is real (so the surface can refuse a name that
// arrived before its body).
...
// dispatch, and a friend who types always dispatches. scrollTop each frame
// cannot be abandoned but by us.
...
// The viewport half alone — a place arriving, not a jump cut. Geometry off
// the height map (`lineBlockAt`), never `coordsAtPos`: a friend twenty screens
// away is not rendered, which is exactly when following matters. scrollTop and
...
// THE FIREFLY — the edge the friend was lost past, and the way back:
// direction and distance. A gutter mark only says "someone is on this line"
// about a line you cannot see. Click resumes (amends D021).
//
...
// is always one frame behind. Out here it does not move, so nothing is late.
...
export function mountChase(view, { initials = () => null, onResume } = {}) {
    ...
    // mousedown must not bubble into outer's hand handler — that would break
    // follow before click re-enters it. Click alone returns to follow.
    ...
    // corners — fade through the rush honestly. Out fast, back slow; settling
